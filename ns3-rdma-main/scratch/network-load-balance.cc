...
void qp_finish(FILE *fout, Ptr<RdmaQueuePair> q) {
    static uint32_t qp_finish_call_count = 0;
    qp_finish_call_count++;

    std::cout<<"流完成"<<std::endl;
    ...
    uint32_t head_bytes = CustomHeader::GetStaticWholeHeaderSize() - IntHeader::GetStaticSize();
    std::cout<<"head_bytes:"<<head_bytes<<std::endl;
    ...
    uint64_t times = q->times - q->m_size / 8192;
    // XXX: remove rxQP from the receiver
    rdma->m_rdma->DeleteRxQp(q->sip.Get(), q->sport, q->dport, q->m_pg);

    // fprintf(fout, "%lu QP complete\n", Simulator::Now().GetTimeStep());
    fprintf(fout, "%u %u %u %u %lu %lu %lu %lu %lu %.5f %.5f %u\n", Settings::ip_to_node_id(q->sip),
            Settings::ip_to_node_id(q->dip), q->sport, q->dport, q->m_size,
            q->startTime.GetTimeStep(), (Simulator::Now() - q->startTime).GetTimeStep(),
            ecn_count, standalone_fct, throughput_bps, goodput_bps, times);

    // if (qp_finish_call_count == 3) {
    //     fprintf(fout, "%lu %lu %lu %lu\n", g_udpPktCntNode[0],
    //             g_udpPktCntNode[1], g_udpPktCntNode[2], g_udpPktCntNode[3]);
    // }

    // for debugging
    std::cout << Settings::ip_to_node_id(q->sip) << " " << Settings::ip_to_node_id(q->dip) << " " 
              << q->sport << " " << q->dport << " 90% FCT: " << ecn_count << " ns" << std::endl;
    NS_LOG_DEBUG("%u %u %u %u %lu %lu %lu %lu %lu\n" %
                 (Settings::ip_to_node_id(q->sip), Settings::ip_to_node_id(q->dip), q->sport,
                  ...
                  (Simulator::Now() - q->startTime).GetTimeStep(), ecn_count, standalone_fct));
