...
uint64_t g_udpPktCntNode[4] = {0, 0, 0, 0};
static const Time g_udpPktCntResetInterval = NanoSeconds(6599336);
static EventId g_udpPktCntResetEvent;

static void ResetUdpPktCntNode() {
    std::cout << "Reset UDP pkt count for each node" << std::endl;
    for (uint32_t i = 0; i < 4; ++i) {
        g_udpPktCntNode[i] = 0;
    }
    g_udpPktCntResetEvent =
        Simulator::Schedule(g_udpPktCntResetInterval, &ResetUdpPktCntNode);
}

static void EnsureUdpPktCntResetScheduled() {
    if (!g_udpPktCntResetEvent.IsRunning()) {
        g_udpPktCntResetEvent =
            Simulator::Schedule(g_udpPktCntResetInterval, &ResetUdpPktCntNode);
    }
}
...
void QbbNetDevice::Receive(Ptr<Packet> packet) {
    ...
    // Count UDP packets for node id 0/1/2/3.只在节点5上
    // if (m_node->GetId() == 0 || m_node->GetId() == 1 || m_node->GetId() == 2 ||
    //     m_node->GetId() == 3) {
    //     if (ch.l3Prot == 0x11) {                // UDP
    //         uint32_t nodeId = m_node->GetId();  // Assuming sip is the source IP address
    //         std::cout << "Node " << nodeId << " received a UDP packet." << std::endl;
    //         if (nodeId < 4) {
    //             g_udpPktCntNode[nodeId]++;
    //         }
    //     }
    //     // for (auto& i : g_udpPktCntNode) {
    //     //     std::cout << i << " ";
    //     // }
    //     // std::cout << std::endl;
    // }
    // 打印某个流的路径
    if (ch.sip == 184550913 + 512 * 1) {
        std::cout << m_node->GetId() << "节点调用接受数据包" << std::endl;
    }
...
bool QbbNetDevice::TransmitStart(Ptr<Packet> p, Ptr<RdmaQueuePair> lastQp) {
    ...
    Simulator::Schedule(txCompleteTime, MakeEvent(&QbbNetDevice::TransmitComplete, this, lastQp));
