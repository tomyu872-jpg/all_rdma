...
bool SwitchMmu::ShouldSendCN(uint32_t ifindex, uint32_t qIndex) {
    if (qIndex == 0)  // qidx=0 as highest priority
        return false;
    // if(m_usedEgressQSharedBytes[ifindex][qIndex]!=0)
    // {
    //     cout<<"端口:"<<ifindex<<"，队列:"<<qIndex<<"，占用的共享缓冲区字节数:"<<m_usedEgressQSharedBytes[ifindex][qIndex]<<endl;
    //     cout<<"最大共享缓冲区字节数:"<<kmax[ifindex]<<endl;
    // }
