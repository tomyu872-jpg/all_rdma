...
class SwitchNode : public Node {
    ...
    static const unsigned pCnt = 128;  // port 0 is not used so + 1	// Number of ports used原来是128
    uint32_t m_ecmpSeed;
   ...
   private:
    ...
    /*----- Load balancer -----*/
    // Flow ECMP (lb_mode = 0)
    uint32_t DoLbFlowECMP(Ptr<Packet> p, CustomHeader &ch,
    ...
    // ConWeave (lb_mode = 9)
    uint32_t DoLbConWeave(Ptr<Packet> p, CustomHeader &ch,
                           const std::vector<int> &nexthops);  // dummy

    // Path index [0, mp_count) marked as deprecated; -1 means no deprecated path.
    int m_abandonedPathId;
    int m_abandonedPathId2;
