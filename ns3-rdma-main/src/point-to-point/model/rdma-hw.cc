...
std::unordered_map<int32_t, uint64_t> RdmaHw::s_flowSizeById;
    ...
    static TypeId tid =
        TypeId("ns3::RdmaHw")
            ...
            .AddAttribute("L2BackToZero", "Layer 2 go back to zero transmission.",
                          ...
                          MakeBooleanChecker())
            .AddAttribute(
                "EwmaGain", "Control gain parameter which determines the level of rate decrease",
                DoubleValue(1.0 / 8), MakeDoubleAccessor(&RdmaHw::m_g), MakeDoubleChecker<double>())
            .AddAttribute("RateOnFirstCnp", "the fraction of rate on first CNP", DoubleValue(1),
            ...
            .AddAttribute("RateDecreaseInterval", "The interval of rate decrease check",
                          DoubleValue(2.0), MakeDoubleAccessor(&RdmaHw::m_rateDecreaseInterval),
                          MakeDoubleChecker<double>())
            .AddAttribute("FastRecoveryTimes", "The rate increase timer at RP", UintegerValue(1),
                          MakeUintegerAccessor(&RdmaHw::m_rpgThreshold),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("AlphaResumInterval", "The interval of resuming alpha", DoubleValue(0.25),
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader& ch) {
    ...
    PathIdTag pathIdTag;
    if (p->PeekPacketTag(pathIdTag)) {
        // cout << "------------接收端------------" << endl;
        // cout << "接收端收到的tag.pathId:" << static_cast<uint32_t>(pathIdTag.GetPathId()) << endl;
        // cout << "接收端收到的seq:" << ch.udp.seq / 8192 << endl;
    }
    ...
    // cout << "RdmaHw::ReceiveUdp中x=" << x << endl;
    if (x == 1 || x == 2 || x == 6) {  // generate ACK or NACK
        ...
        if (ch.sip != 184550913 + 512 * 4) {
            // cout << "接收端发送ack的.pathId:" << static_cast<uint32_t>(pathIdTag.GetPathId())
            //      << endl;
            seqh.SetPathId(static_cast<uint32_t>(pathIdTag.GetPathId()));
        }
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader& ch) {
    ...
    uint32_t sip = ch.sip;
    // cout << "接收端收到ACK, port=" << port << " sport=" << sport << " qIndex=" << qIndex << endl;
    uint32_t pathId = static_cast<uint32_t>(ch.ack.pathId);
    // if (ch.dip != 184550913 + 512 * 4) {
    if (1) {
        // cout << "------------发送端------------" << endl;
        // cout << "发送端收到ACK的pathid:" << pathId << endl;
        if (frist_ack == 0) {
            frist_ack = 1;
        } else {
            if (pathId < kPathChoiceCount) {
                uint64_t qpKey = GetPathStatsQpKey(ch.sip, port, sport, qIndex);
                GetPathIdCountAck(qpKey)[pathId]++;
            }
        }
    }
    // cout << "接收端的sip:" << ch.sip << endl;
    ...
    if (m_irn) {
        ...
        cout<<"收到nack"<<endl;
        if (qp->snd_una > qp->m_lastNackRecoverySeq) {
            qp->m_lastNackRecoverySeq = qp->snd_una;
            // Keep the old send frontier so we can resume normal sending after one repair packet.
            if (!qp->resend_mode) {
                qp->irn.m_recovery_seq = qp->snd_nxt;
                qp->resend_mode = true;
            }
...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t size,
    ...
    if (q->m_flow_id >= 0) {
        auto it = s_flowSizeById.find(q->m_flow_id);
        if (it != s_flowSizeById.end()) {
    ...
    uint32_t sack_seq = 0, sack_len = 0;
    // if(q->m_flow_id==0){
    ...
    if (seq == expected || (seq < expected && seq + size >= expected)) {
        ...
        q->ReceiverNextExpectedSeq += size - (expected - seq);
        bool drained_ooo_blocks = false;
        while (q->m_irn_sack_.peekFrontBlock(&sack_seq, &sack_len) &&
               sack_seq <= q->ReceiverNextExpectedSeq) {
            drained_ooo_blocks = true;
            q->ReceiverNextExpectedSeq += (sack_len - (q->ReceiverNextExpectedSeq - sack_seq));
            q->m_irn_sack_.discardUpTo(q->ReceiverNextExpectedSeq);
        }
        if (drained_ooo_blocks) {
            return 1;
        }
        ...
        // Non-IRN: out-of-order packets are treated as reordering, not loss.
        // Buffer the block and wait for cumulative ACK progress when the gap is filled.
        return 1;
    } else {
        // Duplicate.
        if (m_irn) {
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp) {
    ...
    // if (qp->sip.Get() == 184550913) {
    //     cout << "发送的seq:" << seq / 8192 << endl;
    // }
    ...
    // For non-IRN NACK recovery: after retransmitting snd_una once, jump back to the
    // previously reached send frontier instead of replaying the whole old window.
    if (qp->resend_mode) {
        if (qp->snd_nxt < qp->irn.m_recovery_seq) {
...
void RdmaHw::cnp_received_mlx(Ptr<RdmaQueuePair> q) {
    ...
    if (q->mlx.m_first_cnp) {
        if (static_cast<int>(q->sip.Get()) == 184550913) {
            cout << "q->m_rate1:" << q->m_rate << endl;
        }
        // init alpha
        q->mlx.m_alpha = 1;
