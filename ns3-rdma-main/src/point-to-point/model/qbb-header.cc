...
namespace ns3 {
	...
	qbbHeader::qbbHeader(uint16_t pg)
		: m_pg(pg), sport(0), dport(0), flags(0), m_seq(0), m_pathId(0)
	{
	}

	qbbHeader::qbbHeader()
		: m_pg(0), sport(0), dport(0), flags(0), m_seq(0), m_pathId(0)
	{}
	...
	void qbbHeader::SetPathId(uint8_t pathId){
		m_pathId = pathId;
	}
	...
	{
		return GetBaseSize() + IntHeader::GetStaticSize() + sizeof(m_pathId);
	}
