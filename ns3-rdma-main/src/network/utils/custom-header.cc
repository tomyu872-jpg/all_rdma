...
void CustomHeader::Serialize(Buffer::Iterator start) const {
    ...
    if (headerType & L3_Header) {
        ...
        if (ipv4Flags & DONT_FRAGMENT) flagsFrag |= (1 << 6);
        if (ipv4Flags & MORE_FRAGMENTS) flagsFrag |= (1 << 5);
        i.WriteU8(flagsFrag);
...
uint32_t CustomHeader::Deserialize(Buffer::Iterator start) {
    ...
    if (headerType & L3_Header) {
        ...
        if (brief) {
            ...
            ipv4Flags = 0;
            if (flags & (1 << 6)) ipv4Flags |= DONT_FRAGMENT;
            if (flags & (1 << 5)) ipv4Flags |= MORE_FRAGMENTS;
            i.Prev();
    ...
    if (headerType & L4_Header) {
        if (l3Prot == 0x6) {  // TCP
            ...
            // else
            //     i.Next(IntHeader::GetStaticSize());
            // 读 pathId
            udp.pathId = i.ReadU8();
