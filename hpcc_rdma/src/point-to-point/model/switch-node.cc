    ...
    static TypeId tid =
        TypeId("ns3::SwitchNode")
            ...
            .AddAttribute("AckHighPrio", "Set high priority for ACK/NACK or not", UintegerValue(0),
                          ...
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("IntTrace", "Print INT telemetry inserted at switch egress",
                          BooleanValue(false), MakeBooleanAccessor(&SwitchNode::m_intTrace),
                          MakeBooleanChecker());
    return tid;
}
...
uint32_t SwitchNode::DoLbFlowECMP(Ptr<const Packet> p, const CustomHeader &ch,
    ...
    if (ch.l3Prot == 0x6)
        ...
        // Fallback for non-TCP/UDP and non-ACK control packets (e.g., ICMP):
        // keep deterministic ECMP by hashing only on src/dst IP.
        buf.u32[2] = 0;
    }
...
void SwitchNode::SendToDevContinue(Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (idx >= 0) {
        ...
        if (ch.l3Prot == 0xFF || ch.l3Prot == 0xFE ||
            ...
            qIndex = 1;  // TCP goes to queue 1
        } else if (ch.l3Prot == 0x11) {
            qIndex = ch.udp.pg;  // UDP data packets use the packet group
        } else {
            qIndex = 0;
        }
    ...
    if (1) {
        ...
        if (buf[PppHeader::GetStaticSize() + 9] == 0x11) {  // udp packet
            ...
            if (m_ccMode == 3) {  // HPCC
                ...
                if (m_intTrace) {
                    uint32_t idx = (ih->nhop - 1) % IntHeader::maxHop;
                    const IntHop &h = ih->hop[idx];
                    std::cout << "[INT_PUSH] t=" << Simulator::Now().GetNanoSeconds()
                              << " sw=" << m_id << " outDev=" << ifIndex << " q=" << qIndex
                              << " nhop=" << ih->nhop << " hopIdx=" << idx
                              << " qlen=" << h.GetQlen() << " bytes=" << h.GetBytes()
                              << " time=" << h.GetTime() << " lineRate=" << h.GetLineRate()
                              << " pktSize=" << p->GetSize() << std::endl;
                }
            }
        }
    }
