...
void ConWeaveRouting::RouteInput(Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (ch.l3Prot != 0x11 && ch.l3Prot != 0xFD && ch.l3Prot != 0xFC) {
        SLB_LOG(PARSE_FIVE_TUPLE(ch) << "ACK/PFC or other control pkts -> do flow-ECMP. Sw("
                                     << m_switch_id << "),l3Prot:" << ch.l3Prot);
        DoSwitchSendToDev(p, ch);
        return;
    }
    assert(ch.l3Prot == 0x11 || ch.l3Prot == 0xFD || ch.l3Prot == 0xFC);  // UDP or ACK/NACK packets
    ...
    if (ch.l3Prot == 0xFD || ch.l3Prot == 0xFC) { /** ACK/NACK or ConWeave's Control Packets */
