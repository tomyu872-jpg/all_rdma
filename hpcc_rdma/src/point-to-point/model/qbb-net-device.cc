...
int RdmaEgressQueue::GetNextQindex(bool paused[]) {
    ...
    for (qIndex = 1; qIndex <= fcount; qIndex++) {
        ...
        bool cond2 = ((qp->GetBytesLeft() > 0 || hasPendingRetrans) && cond_window_allowed);
...
bool QbbNetDevice::Send(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber) {
    ...
    // Host path: reuse existing high-priority enqueue + dequeue machine.
    if (m_node->GetNodeType() == 0) {
        m_macTxTrace(p);
        m_traceEnqueue(p, 0);
    ...
    // Switch path: preserve queue priority semantics used by existing switch send path.
    uint32_t qIndex = 1;
