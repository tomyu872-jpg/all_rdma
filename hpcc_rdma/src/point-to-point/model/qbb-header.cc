...
namespace ns3 {
	...
	qbbHeader::qbbHeader(uint16_t pg)
		: m_pg(pg), sport(0), dport(0), flags(0), m_seq(0), m_irn_nack(0),
		  m_irn_nack_size(0), m_falcon_bitmap_bits(0), m_falcon_bitmap_hi(0),
		  m_falcon_bitmap_lo(0)
	{
	}

	qbbHeader::qbbHeader()
		: m_pg(0), sport(0), dport(0), flags(0), m_seq(0), m_irn_nack(0),
		  m_irn_nack_size(0), m_falcon_bitmap_bits(0), m_falcon_bitmap_hi(0),
		  m_falcon_bitmap_lo(0)
	{}
	...
	void qbbHeader::SetDirectNack(){
		flags |= 1 << FLAG_DIRECT_NACK;
	}
	void qbbHeader::SetFalconBitmap(uint64_t bitmap, uint16_t bits){
		if (bits != 0){
			flags |= 1 << FLAG_FALCON_BITMAP;
		}
		m_falcon_bitmap_bits = bits;
		m_falcon_bitmap_hi = static_cast<uint32_t>(bitmap >> 32);
		m_falcon_bitmap_lo = static_cast<uint32_t>(bitmap & 0xffffffffu);
	}
	...
	uint8_t qbbHeader::GetDirectNack() const{
		return (flags >> FLAG_DIRECT_NACK) & 1;
	}
	uint64_t qbbHeader::GetFalconBitmap() const{
		return (static_cast<uint64_t>(m_falcon_bitmap_hi) << 32) | m_falcon_bitmap_lo;
	}
	uint16_t qbbHeader::GetFalconBitmapBits() const{
		return m_falcon_bitmap_bits;
	}
	...
	uint32_t qbbHeader::GetBaseSize() {
		...
		return sizeof(tmp.sport) + sizeof(tmp.dport) + sizeof(tmp.flags) + sizeof(tmp.m_pg) +
		       sizeof(tmp.m_seq) + sizeof(tmp.m_irn_nack) + sizeof(tmp.m_irn_nack_size) +
		       sizeof(tmp.m_falcon_bitmap_bits) + sizeof(tmp.m_falcon_bitmap_hi) +
		       sizeof(tmp.m_falcon_bitmap_lo);
	}
	void qbbHeader::Serialize(Buffer::Iterator start)  const
	{
		...
		i.WriteU16(m_falcon_bitmap_bits);
		i.WriteU32(m_falcon_bitmap_hi);
		i.WriteU32(m_falcon_bitmap_lo);
	...
	uint32_t qbbHeader::Deserialize(Buffer::Iterator start)
	{
		...
		m_falcon_bitmap_bits = i.ReadU16();
		m_falcon_bitmap_hi = i.ReadU32();
		m_falcon_bitmap_lo = i.ReadU32();
