  ...
  if (firstTime)
    {
      if (! g_markingTimes)
        {
          // Keep the registry alive for the lifetime of the process. Under
          // heavy Time churn in this fork, tearing down the tree itself is
          // fragile; before Simulator::Run we only need to stop tracking.
          static MarkedTimes * markingTimes = new MarkedTimes ();
