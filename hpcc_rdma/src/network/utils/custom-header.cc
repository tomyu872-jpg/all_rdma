...
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  i.WriteU16(ack.falconBitmapBits);
		  i.WriteU32(ack.falconBitmapHi);
		  i.WriteU32(ack.falconBitmapLo);
		  udp.ih.Serialize(i);
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  ack.falconBitmapBits = i.ReadU16();
		  ack.falconBitmapHi = i.ReadU32();
		  ack.falconBitmapLo = i.ReadU32();
		  if (getInt)
...
uint32_t CustomHeader::GetAckSerializedSize(void){
	return sizeof(ack.sport) + sizeof(ack.dport) + sizeof(ack.flags) + sizeof(ack.pg) +
	       ...
	       sizeof(ack.falconBitmapBits) + sizeof(ack.falconBitmapHi) +
	       sizeof(ack.falconBitmapLo) +
